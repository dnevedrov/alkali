...
C     ================================================    
      DO M=1,NT
         X=AMP(M)*1.0D8
         IF (DIREC.EQ.'100') THEN
            Y=0.91215D0*X**2+0.4665D0*X+0.78399D0
            WL(M)=Y*WMAX
         ELSEIF (DIREC.EQ.'110') THEN
            Y=0.40200D0*X*X+0.43220D0*X+0.73823D0
            WL(M)=Y*WMAX
         ELSEIF (DIREC.EQ.'111') THEN
            Y=0.30033D0*X*X++0.27708D0*X+0.77972D0
            WL(M)=Y*WMAX
         ENDIF
         NT1=M
         IF (WL(M)/WMAX.LT.1.0001) GOTO 20
     ...
     *        (1.0D0/64.0D0)*K4*AMP(M)**4
         ...
         NL=WL(M)/WMAX*(NW/2)
         DO K=N1,N2            
            GAM=DK2/MCL+(1.0D0/12.0D0)*K4*AMP(M)**2/MCL
