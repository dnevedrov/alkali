      ...
      PARAMETER (NW=1129,NT=1000,NM=9)
     ...
     *    X01AAF,T(NT),DT,GAMMA(NT),B,K1,K2,K3,K4,AMP(NT),INC,X,wmax1,
     *    wmax2,
     *    AU,DK2,MD,MCL,MK,WMAX,R0,GAM,KK3,Y, k20,k30,k40, nfac, sfac
      CHARACTER*1 TR
      CHARACTER*3 DIREC
C     ------------------------------------------------
      DIREC='100'
...
C     ================================================    
      DO M=1,nt
         X=AMP(M)*1.0D10
         IF (DIREC.EQ.'100') THEN
          y = 3.0436d0*x**4 - 8.8995d0*x**3 + 9.6557d0*x**2 - 
     *	4.6563d0*x + 1.7046d0
            WL(M)=Y*WMAX1
	
	
         ELSEIF (DIREC.EQ.'110') THEN
		y = 2.8036d0*x**4 - 9.3383d0*x**3 + 11.782d0*x**2 - 6.7595d0*x
     *	 + 2.3701d0	

            WL(M)=Y*WMAX1
         ELSEIF (DIREC.EQ.'111') THEN
	   y = 3.0033d0*x**4 - 10.972d0*x**3 + 15.356d0*x**2 - 9.8914d0*x 
     *	   + 3.3837d0

            WL(M)=Y*WMAX1
         ENDIF
cc		print*,'w = ', amp(m), y, wl(m)/wmax
         NT1=M
         IF (WL(M)/WMAX1.GT.0.99d0) GOTO 20
         E(M)=(AMP(M)*WL(M))**2*MD/2.0D0
c     *      +  (1.0D0/64.0D0)*K4*AMP(M)**4.
c         N1=(-(WL(M)-WMAX)/WMAX)*(NW/2)
		n1 = (wl(m)-wmax)/wmax*277
...
c         AMP(M+1)=DSQRT(2.0D0*E(M+1)/(MD*WL(M)**2))
         IF (M.LE.NT-1) T(M+1)=(T(M)+DT*WMAX2/(2.0D0*PI))
         PRINT*,M,AMP(M),gamma(m)/e(m)/wmax2
	if ((gamma(m)/e(m)).gt.wmax2) then
c	gamma(m) = wmax2*e(m) 
	print*,gamma(m)
	endif   
20    ENDDO
...
C     ================================================================      
      ...
      SUBROUTINE RYBA(NT1,T,G,E,A,DIREC)
      INTEGER K,NT1
      DOUBLE PRECISION T(NT1),G(NT1),A(NT1),E(NT1)
      CHARACTER*3 DIREC
      OPEN(1,FILE=DIREC)
      DO K=1,NT1
         WRITE(1,3)T(K),G(K)/E(K)/2.6928d13,A(K)*1.0d10,E(K)
      ENDDO
      CLOSE(1)
 3    FORMAT(4E16.8)
      RETURN
