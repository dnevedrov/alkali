...
c      READ*,AMP
	...
	ENDIF
      ...
      MK=39.0D0*AU
	      MD=MK*AU
 30   AMP=AMP-INC        
      GAM=DK2/MCL+(1.0d0/12.0d0)*K4*AMP**2/MCL
...
C	      ftor =  (1.0d0+(K3**2/(2.0d0*K4))*G0)
		  FTOR = (1.0d0+(K3**2/(K4*MCL))*91.966D0*(F10-F20))
			PRINT*,MCL/(91.966D0*F10)
...
C     ================================================
      CALL MAZ(TMP,M)
      PRINT*,'AMP   ',AMP*1.0D8,OMEGA(M)/WMAX,  FTOR
      WRITE(322,1222)AMP*1.0d8,OMEGA(M)/WMAX,ftor
      L=L+1

      IF (L.LE.600.AND.COND.EQ.1) GOTO 30
      PRINT*,'COMPLETED'
      CLOSE(322)
      END
 ...
 2    FORMAT(3E16.8)
      ...
      INTEGER I1,I2,I
      COMPLEX*16 GG(9,9),TMP(9,9),GG0(9,9),G(36,1689),DC,W(9,9),
     *     AL,BE,KOFF,GG1(3,3),GG2(3,3),GG3(3,3)
      CHARACTER*1 TR
      AL=DCMPLX(1.0D0,0.0D0)
      BE=DCMPLX(0.0D0,0.0D0)
      TR='N'
      KOFF=DCMPLX(91.966D0,0.0D0)
      GG1(1,1) = KOFF*G(12,I)
      GG1(1,2) = KOFF*G(10,I)
      GG1(1,3) = KOFF*G(10,I)
      GG1(2,1) = KOFF*G(10,I)
