      PROGRAM KCL
C     ======================================================
C     THE PROGRAM CALCULATES THE DYNAMICS IMPURITY CENTERS
C     IN ALKALI HALIDES.
...
C     ======================================================
      INTEGER K,J,I,NT,P01ABF,IFAIL,N1,N2,NL,M,NM,NW
      PARAMETER (NW=1689,NT=3000,NM=9)
     ...
     *    X01AAF,T(NT),DT,GAMMA(NT),KAPPA,B,K1,K2,K3,K4,AMP(NT),INC,X,
     *    AU,DK2,MD,MCL,MK,WMAX,R0,GAM,KK3,Y
      CHARACTER*1 TR
      CHARACTER*3 DIREC
C     ------------------------------------------------
      DIREC='111'
...
C     ================================================    
      DO M=1,NT
         X=AMP(M)*1.0D8
         IF (DIREC.EQ.'100') THEN
            Y=1.090084994D0*X*X+0.2958893033D0*X+
     *           1.080854270D0
            WL(M)=Y*WMAX
         ELSEIF (DIREC.EQ.'110') THEN
            Y=0.5434868861D0*X*X+0.2107588401D0*X+
     *           1.080669043D0
            WL(M)=Y*WMAX
         ELSEIF (DIREC.EQ.'111') THEN
            Y=0.3641539892D0*X*X++0.1700242735D0*X+
     *           1.081076318D0
            WL(M)=Y*WMAX
         ENDIF
         IF (M.EQ.1) E(M)=(AMP(M)*WL(M))**2*MD/2.0D0+
     *        (1.0D0/64.0D0)*K4*AMP(M)**4
         N1=((WL(M)-WMAX)/WMAX)*(NW/2)
         N2=(NW/2)
         NL=WL(M)/WMAX*(NW/2)
         DO K=N1,N2            
            GAM=DK2/MCL+(1.0D0/4.0D0)*K4*AMP(M)**2/MCL
...
C     ================================================================
         ...
         GAMMA(M)=DABS(RESULT)*(HBAR*WL(M)*0.5D0/PI)*(AMP(M)*KK3)**2
         E(M+1)=E(M)*0.993D0
         ...
         ENDIF
         AMP(M+1)=DSQRT(2.0D0*E(M+1)/(MD*WL(M)**2))
         IF (M.LE.NT-1) T(M+1)=(T(M)+DT*WMAX/(2.0D0*PI))
         PRINT*,(GAMMA(M)/E(M))/WMAX,T(M+1)-T(M),GAMMA(M)
      ENDDO
C     ================================================================      
      CALL RYBA(NT,T,GAMMA,E,AMP,DIREC)
C     ================================================================      
      END
 ...
 2    FORMAT(3E16.8)
     ...
     *     AL,BE,KOFF,GG1(3,3),GG2(3,3),GG3(3,3)
      ...
      BE=DCMPLX(0.0D0,0.0D0)
      TR='N'
      KOFF=DCMPLX(92.68146641096322D0,0.0D0)
      DO I1=1,9
         DO I2=1,9
