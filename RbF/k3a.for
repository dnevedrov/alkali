c     last updated 06/09/2000 by Dmitri Nevedrov

      PROGRAM KCL
      INTEGER L,K,J,M,I,NM
      PARAMETER (NW=1115,NM=9)
      COMPLEX*16 DC,G(36,NW),GG(9,9),W(9,9),W1(3,3),W2(3,3),W3(3,3)
      DOUBLE PRECISION OMEGA(NW),K1,K2,K3,K4,AMP,AU,DK2,MD,MCL,MK,R0,
     *     INC,TMP(NW),GAM,B,WMAX,ftor,F10,F20, K20, K30, K40
	CHARACTER*3 CRYST
      DOUBLE PRECISION G0,M1,M2,G11,G22,G12
	INTEGER COND
	COND = 0 
c	read greens functions
      CALL KIRGUDU(OMEGA,G)
	CRYST = '111'
      OPEN(322,FILE='RBF111.DAT')
c	top phonon frequency
      WMAX=3.03D13
      AU=1.6605655D-27
c	starting amplitude
      AMP=1.0D-10
c	increment
      INC=1.0D-10/100.0D0
      L=1
...
c	masses
 ...
 30   AMP=AMP-INC  
      ...
      GAM=DK2/MCL+(1.0d0/12.0d0)*K4*AMP**2/MK*(-3.3207D0)
...
C     ================================================    
      DO K=1,NW
	   B=(1.0D0-MD/MK)*OMEGA(K)**2+2.0D0*GAM    
...
C	      ftor =  (1.0d0+(K3**2/(2.0d0*K4))*G0)
		  FTOR = (1.0d0+(K3**2/(K4*MK))*94.76*(F10-F20))
			PRINT*, FTOR
...
C     ================================================
 ...
 2    FORMAT(3E16.8)
      ...
      INTEGER I1,I2,I
      COMPLEX*16 GG(9,9),TMP(9,9),GG0(9,9),G(36,1115),DC,W(9,9),
     *     AL,BE,KOFF,GG1(3,3),GG2(3,3),GG3(3,3)
      CHARACTER*1 TR
      AL=DCMPLX(1.0D0,0.0D0)
      BE=DCMPLX(0.0D0,0.0D0)
      TR='N'
      KOFF=DCMPLX(94.76D0,0.0D0)
      GG1(1,1) = KOFF*G(12,I)
      GG1(1,2) = KOFF*G(10,I)
      GG1(1,3) = KOFF*G(10,I)
      GG1(2,1) = KOFF*G(10,I)
...
C
      ...
      SUBROUTINE MAZ(X,I)
      INTEGER J
      DOUBLE PRECISION X(1115),TMP
      TMP=0.0D0
	open(333,file='plot.txt')
	do j=252,325
	write(333,10101)j,x(j)
	enddo
	close(333)
10101 format(2e16.8)
      DO J=252,325
