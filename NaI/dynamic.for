c     last updated 05/22/2000 by Dmitri Nevedrov

      PROGRAM KCL
      INTEGER L,K,J,M,I,NM
      PARAMETER (NW=1375,NM=9)
      COMPLEX*16 DC,G(36,NW),GG(9,9),W(9,9),W1(3,3),W2(3,3),W3(3,3)
      DOUBLE PRECISION OMEGA(NW),K1,K2,K3,K4,AMP,AU,DK2,MD,MCL,MK,R0,
     *     INC,TMP(NW),GAM,GAM1,B,WMAX,ftor,F10,F20, K20, K30, K40,
     *     nk2,nk1,nk3,nk4,nk20,nk30,nk40
	CHARACTER*3 CRYST
      DOUBLE PRECISION G0,M1,M2,G11,G22,G12, POTS(4), xi
      CALL KIRGUDU(OMEGA,G)
	CRYST = '100'
      OPEN(322,FILE='NAI100.DAT')
      WMAX=2.17D13
      AU=1.6605655D-27
      AMP=1.0D-10
      INC=AMP/100.0D0
	...
	IF (CRYST.EQ.'100') THEN
	K3 = K3
	K4 = K4
	ELSEIF (CRYST.EQ.'110') THEN
	K3 = K3 + K30
	K4 = K4 + K40
	ELSEIF (CRYST.EQ.'111') THEN
	K3 = K3 + 2.0D0*K30
	K4 = K4 + 2.0D0*K40
	ENDIF
      DK2=0.0D-6
...
c	PRINT*,K2,K3,K4
	NK20 = NK1/R0
	NK30 = (NK2 - NK20)/R0
	NK40 = NK3/R0 - (NK2-NK20)/(2.0D0*R0*R0)
	IF (CRYST.EQ.'100') THEN
	NK3 = NK3
	NK4 = NK4
	ELSEIF (CRYST.EQ.'110') THEN
	...
	ENDIF
      ...
      GAM=DK2/MCL+(1.0d0/8.0d0)*K4*AMP**2/MCL*(ftor)
	GAM=DK2/MCL+(1.0d0/4.0d0)*NK4*AMP**2/MCL + (NK2 - K2)/MCL
	print*,gam
C     *	 *(1.0d0-(K3**2/(K2*K4)))
C       PRINT*,(1.0d0-(K3**2/(K2*K4)))
 100  DO J=1,9
         DO I=1,9
...
C     ================================================    
      DO K=1,NW
	   B=(1.0D0-MD/MCL)*OMEGA(K)**2+2.0D0*GAM    
         ...
         CALL TIGR(GG,G,K,W) 
         if (k.eq.1) then
            G11 = DREAL(GG(1+6,1+6))
            G12 = DREAL(GG(1+6,2+6))
            G22 = DREAL(GG(2+6,2+6))
		  F10 = DREAL(G(1,1))
		  F20 = DREAL(G(2,1))
            G0 = G11/M1 + G22/M2 - 2.0D0*G12/DSQRT(4.0d0*M1*M2)
	      ftor =  (1.0d0+(K3**2/(MCL*K4))*95.33D0*(F10-F20))

2560	xi = 0.25d0* amp**2 *k3* (f10-f20)/M2 
...
C		  FTOR = (1.0d0-(K3**2/(K2*K4)))
	if (xi/0.25d0* amp**2 *k3* (f10-f20)/M2-1.0d0.gt.0.001d0) then
	goto 2560
	endif
c	print*, xi, ftor
            GAM1=DK2/MCL+(1.0d0/12.0d0)*K4*AMP**2/MCL*ftor
			if (GAM1.LT.0.0d0) then
				COND = 0
			ENDIF
			if (GAM1.GT.0.0d0) then
				COND = 0
			ENDIF
			IF (COND.EQ.1) THEN
			   B=(1.0D0-MD/MCL)*OMEGA(K)**2+2.0D0*GAM1    
			   W1(1,1)=DCMPLX(B,0.0D0)
		       W1(2,2)=DCMPLX(GAM,0.0D0)
		       W1(3,3)=DCMPLX(GAM,0.0D0)
			   W1(1,2)=DCMPLX(-GAM,0.0D0)
      ...
      ENDDO
C     ================================================
c     OPEN(233,FILE='TEST')
c     DO I=1,NW
c     WRITE(233,1222)OMEGA(I)/WMAX,TMP(I)*1.0D26
c     ENDDO
c     CLOSE(233)
 1222 FORMAT(3E16.8)
C     ================================================
      CALL MAZ(TMP,M)
      PRINT*,'AMP   ',AMP*1.0D10,OMEGA(M)/WMAX
      WRITE(322,1222)AMP*1.0d10,OMEGA(M)/WMAX
      L=L+1
     ...
     *     AL,BE,KOFF,GG1(3,3),GG2(3,3),GG3(3,3)
      ...
      DO I1=1,3
         DO I2=1,3
            ...
            GG0(I1+6,I2+6)=GG3(I1,I2)
         ENDDO
      ENDDO      
       DO I1=1,9
         DO I2=1,9
            TMP(I1,I2)=DCMPLX(0.0D0,0.0D0)
         ENDDO
      ENDDO
...
C
      ...
      SUBROUTINE MAZ(X,I)
      INTEGER J
      DOUBLE PRECISION X(1375),TMP
      TMP=0.0D0
c	OPEN(333,FILE='TEMP.TXT')
c	DO j=309,466
c	WRITE(3,1031)J,X(J)
c	ENDDO
c	CLOSe(333)
c1031  FORMAT(2E16.8)
      DO J=309,466
         IF (DABS(X(J)).GT.TMP) THEN
            TMP=DABS(X(J))
            I=J
         ENDIF
      ENDDO
...
99999 FORMAT (A)
	...
	SUBROUTINE POTENTIAL(R0,POTS)

      INTEGER          NOUT
      PARAMETER        (NOUT=6)
*     .. Local Scalars ..
      DOUBLE PRECISION HBASE, XVAL, R0
      INTEGER          I, IFAIL, J, K, L, NDER, I1
*     .. Local Arrays ..
      DOUBLE PRECISION DER(14), POTS(4), EREST(14)
*     .. External Functions ..
...
c     .. Scalar Arguments ..
      DOUBLE PRECISION DAT(5,16),X0,X
	DOUBLE PRECISION EL, LAMBDA, RHO, EPS0
	INTEGER I,J,L
     ...
     *2.989d0,1.33D-8,0.328D0,0.0D0, 0.0D0,
     *3.237d-10,1.58D-8,0.345,0.0D0, 0.0D0,
     *2.674d-10,45.1D-79,137.0D-99,2.43D-58, 1.37D10,
     *3.147d0, 2.05d-8, 0.326d0, 0.0d0, 0.0d0,
     *3.298d-10,132.0D-79,442.0D-99,3.77D-58, 1.52D10,
     *3.533d0,2.85D-8,0.348D0,0.0D0, 0.0D0,
     *2.815d-10,1.78D-8,0.301D0,0.0D0, 0.0D0,
...
30	EL = 1.602176462D-19
	EPS0 = 8.854187817D-12
	X0 = DAT(1,L)
	LAMBDA = DAT(2,L)/1.0D0
	RHO = DAT(3,L)

      FUN = (LAMBDA*DEXP(-X*1.0D10/RHO))*1.0D-7 - EL**2/
     * (4.0D0*3.14D0*EPS0*X)
